...
def _flow() -> SprintFlow:
    return SprintFlow(operator=MagicMock())
...
def test_branch_for_task_includes_key_and_title_slug() -> None:
    ...
    assert branch.startswith("sendsprint/")
    assert "proj-42" in branch
...
def test_branch_for_task_handles_missing_title() -> None:
    ...
    assert branch == "sendsprint/proj-7"
